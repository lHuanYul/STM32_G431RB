...
__weak Result fdcan_pkt_ist_read_inner(FdcanPkt* pkt, uint8_t byte0)
{
    return RESULT_ERROR(RES_ERR_NOT_FOUND);
}

__weak Result fdcan_pkt_rcv_read_inner(FdcanPkt* pkt, uint8_t byte0)
{
    return RESULT_ERROR(RES_ERR_NOT_FOUND);
}
...
Result fdcan_pkt_ist_read(FdcanPkt* pkt)
{
    ...
    switch (code)
    {
        ...
        default: return fdcan_pkt_ist_read_inner(pkt, code);
    }
}
...
static Result fdcan_pkt_rcv_read(FdcanPkt* pkt)
{
    uint8_t code;
    RESULT_CHECK_RET_RES(fdcan_pkt_get_byte(pkt, 0, &code));
    switch (code)
    {
        default: return fdcan_pkt_rcv_read_inner(pkt, code);
    }
}

static UNUSED_FNC Result recv_pkts_proc(size_t count)
{
