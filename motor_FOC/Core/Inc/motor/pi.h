...
typedef struct PI_CTRL {
    ...
    float32_t   out_ori;  // Data: pre-saturated controller output 飽和前的控制輸出
    float32_t   max;      // Parameter: upper saturation limit
    float32_t   min;      // Parameter: lower saturation limit
    float32_t   Kp;       // Parameter: proportional loop gain 比例增益
    float32_t   Ki;       // Parameter: integral gain 積分增益
    float32_t   Delta;    // Data: ref - Fbk
    float32_t   Up;       // Data: proportional term 比例項 Kp * Delta
